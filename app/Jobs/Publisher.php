...
class Publisher extends Job implements SelfHandling
{
    ...
    public function __construct(SettingRepository $settingRepository, PostRepository $postRepository, PublisherQueueRepository $publisherQueueRepository, FacebookService $facebookService)
    {
    ...
    {
        $check = $this->publisherQueueRepository->getFirstItem();
        if (count($check))
        {
            ...
            # rand publish time to escape spam detector
            // if ($delay) {
    ...
    public function directPublish($job, $post_id)
    {
        $post = $this->postRepository->getItemWithoutTrashed($post_id);
        if (count($post) && $post->deleted_at == '')
        {
            # generate and update post true id
            $id = $post->post_id;
            $true_id_base10 = $this->getPostTrueId();
            ...
            # delete if in the queue list
            $check = $this->publisherQueueRepository->getItemByPostId($id);
            if (count($check)) {
                $this->publisherQueueRepository->delete($check->id);
            }

            \Log::info('Direct publish post: #' . $post->id . ', true_id: ' . $true_id . ' ,at ' . currentTime());
